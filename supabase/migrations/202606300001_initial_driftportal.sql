...
create policy "Authenticated users can view portal files"
  on storage.objects for select
  to authenticated
  using (
    bucket_id in ('sds-documents', 'work-order-attachments', 'equipment-photos')
  );
...
create policy "Owners and admins can update portal files"
  on storage.objects for update
  to authenticated
  using (
    bucket_id in ('sds-documents', 'work-order-attachments', 'equipment-photos')
    and (owner_id = auth.uid()::text or public.is_portal_admin())
  )
  with check (
    bucket_id in ('sds-documents', 'work-order-attachments', 'equipment-photos')
    and (owner_id = auth.uid()::text or public.is_portal_admin())
...
create policy "Owners and admins can delete portal files"
